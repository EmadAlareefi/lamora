...
export default function Navbar() {
  ...
  const toggleMenu = () => {
    setIsMenuOpen((prev) => {
      if (!prev) setIsCartVisible(false); // Close cart when opening menu
      return !prev;
    });
  };
  ...
  const toggleCartVisibility = () => {
    setIsCartVisible((prev) => {
      if (!prev) setIsMenuOpen(false); // Close menu when opening cart
      return !prev;
    });
  };
  useEffect(() => {
    ...
    <>
      <nav
        className={`transition-all duration-300 ease-in-out h-[78px] max-h-[78px] min-h-[78px] font-gotham-bold uppercase flex justify-between items-center px-5 py-0 ${
            isHomePage && !isScrolled ? "text-cotton bg-transparent" : "text-wine bg-cotton"
      ...
      >
        ...
        <div className="hidden font-extrabold lg:flex space-x-6 mr-auto text-[#DCAE97]">
          <a href="/new-in">جديد السهرة</a>
          <p
            className="pointer-events-auto whitespace-nowrap cursor-pointer  flex gap-1 items-center"
            onClick={toggleCartVisibility}
          >
            <span className="hover:underline hover:underline-offset-4 3xl:text-[11px]">
              السلة
            </span>
          </p>
        </div>
      </nav>
      <ShoppingBag onClose={toggleCartVisibility} isOpen={isCartVisible} />
    </>
  );
}
